...
while true; do
	...
	for row in $(echo "$hits" | jq -r '.[] | @base64'); do
	    ...
	    inputJson=$(echo $inputJson | jq --arg from "$from_id" --arg to "$to_id" --arg old_tweet "$old_tweet" --arg new_tweet "$new_tweet" ' += [{from_id: $from , to_id: $to, old_tweet: $old_tweet, new_tweet: $new_tweet}]')
	done
	if [ $i -eq 0 ]
 		then
		break
