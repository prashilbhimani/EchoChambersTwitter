first_request=$(curl -X POST "localhost:9200/mentions/_search?scroll=10m" -H 'Content-Type: application/json' -d '{"size": 10,"query": {"match_all" : {}}}' )
...
while true; do
	...
	for row in $(echo "$hits" | jq -r '.[] | @base64'); do
	    ...
	    i=$((i+1))
	    from_id=$(_jq '._source.from')
	    to_id=$(_jq '._source.to')
	    tweet=$(_jq '._source.tweet')
	    query="MATCH (a:User),(b:User) WHERE a.id_str = \"$from_id\" AND b.id_str = \"$to_id\" CREATE (a)-[r:MENTIONS{tweet: \"$tweet\"}]->(b) RETURN r"
